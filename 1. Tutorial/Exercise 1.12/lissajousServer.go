// LissajousServer is a HTTP server that generates animated Lissajous figure GIFs based on the URL parameters
package main

import (
	"fmt"
	"image"
	"image/color"
	"image/gif"
	"io"
	"log"
	"math"
	"math/rand"
	"net/http"
	"strconv"
)

var palette = []color.Color{color.White, color.Black}

const (
	whiteIndex = 0 // first color in palette
	blackIndex = 1 // next color in palette
)

func main() {
	handler := func(w http.ResponseWriter, r *http.Request) {
		cycles := 5   // number of complete x oscillator revolutions
		res := 0.001  // angular resolution
		size := 100   // image canvas covers [-size..+size]
		nframes := 64 // number of animation frames
		delay := 8    // delay between frames in 10ms units
		if err := r.ParseForm(); err != nil {
			fmt.Fprint(w, err)
			return
		}
		for param, values := range r.Form {
			// if a paramater has multiple values use the last one.
			value := values[len(values)-1]
			var err error
			if param == "cycles" {
				cycles, err = strconv.Atoi(value)
			} else if param == "res" {
				res, err = strconv.ParseFloat(value, 64)
			} else if param == "size" {
				size, err = strconv.Atoi(value)
			} else if param == "nframes" {
				nframes, err = strconv.Atoi(value)
			} else if param == "delay" {
				delay, err = strconv.Atoi(value)
			} else {
				fmt.Printf("Ignoring parameter %q with value %s\n", param, value)
			}

			if err != nil {
				fmt.Fprint(w, err)
				return
			}
		}
		lissajous(w, cycles, res, size, nframes, delay)
	}
	http.HandleFunc("/", handler)
	log.Fatal(http.ListenAndServe("localhost:8000", nil))
}

func lissajous(out io.Writer, cycles int, res float64, size int, nframes int, delay int) {
	freq := rand.Float64() * 3.0 // relative frequence of y oscillator
	anim := gif.GIF{LoopCount: nframes}
	phase := 0.0 // phase difference
	for i := 0; i < nframes; i++ {
		rect := image.Rect(0, 0, 2*size+1, 2*size+1)
		img := image.NewPaletted(rect, palette)
		for t := 0.0; t < float64(cycles)*2*math.Pi; t += res {
			x := math.Sin(t)
			y := math.Sin(t*freq + phase)
			img.SetColorIndex(size+int(x*float64(size)+0.5), size+int(y*float64(size)+0.5), blackIndex)
		}
		phase += 0.1
		anim.Delay = append(anim.Delay, delay)
		anim.Image = append(anim.Image, img)
	}
	gif.EncodeAll(out, &anim) // NOTE: Ignoring encoding errors
}
